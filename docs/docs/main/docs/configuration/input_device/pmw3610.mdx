...
# Hold the sensor awake while the keyboard is awake: no REST wake-up delay
# between movements, at the cost of RUN current. Released while the keyboard's
# idle sleep (`split_central_sleep_timeout_seconds`) is active.
force_awake = false
smart_mode = true
cpi = 800
