...
Define a `PointingDevice` and add it to `run_all!` macro. A `PointingDevice`
subscribes to `SleepStateEvent` (it releases the sensor's awake hold while the
keyboard sleeps); sensors declared in `keyboard.toml` are counted into the
`sleep_state` subscriber budget automatically, but a device built by hand needs
its slot declared:

```toml
[event.sleep_state]
subs = 1 # one per hand-built PointingDevice, plus your own subscribers
```
