...
The bootloader divides flash into regions. The defaults follow the bootymcbootface convention (shipped in `bootymcbootface/` in the RMK repo) and are automatically calculated from `flash_size`:
