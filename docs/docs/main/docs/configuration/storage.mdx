...
::: info Storage is erased when the firmware changes

Storage remembers the firmware that wrote it: the `rmk` version and commit, its features, and the compiled-in keymap and defaults. Flashing a firmware that differs in any of these erases the storage and starts from the new defaults; reflashing the same firmware keeps everything.
