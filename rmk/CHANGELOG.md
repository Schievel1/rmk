...
- Make Trouble BLE roles explicit, document environment-variable memory tuning, update the nRF52832 examples to peripheral-only SDC, and derive split notification capacity from Trouble's configured packet-pool MTU.
- Add a `dfu_detach` feature for dongles: a USB DFU runtime interface whose DETACH reboots into the bootloader, so a dongle can be updated with `dfu-util` or rmk-gui although its host protocol is relayed to the keyboard. Honoured only for 30 s after plug-in.
