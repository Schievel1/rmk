...
#+BEGIN_SRC sh
cargo make uf2-2mb       # → bootymcbootface-rp2040-2mb.uf2
cargo make uf2-4mb       # → bootymcbootface-rp2040-4mb.uf2
cargo make uf2-8mb       # → bootymcbootface-rp2040-8mb.uf2
cargo make uf2-16mb      # → bootymcbootface-rp2040-16mb.uf2
cargo make uf2-nrf       # → bootymcbootface-nrf52840.uf2
#+END_SRC
...
**BE AWARE THAT BOOTYMCBOOTFACE OVERWRITES THAT BOOTLOADER ON THE NRF52840!**
...
bootymcbootface's partition layout is the default that RMK's =[dfu]=
...
flash partitioning matches bootymcbootface's =memory.x=.
...
1. Hold BOOTSEL, plug in USB, release. (For nRF52840 connect RESET to GND
   twice within 500 ms — but note that bootymcbootface replaces the Adafruit
   bootloader, so after the first flash you need probe-rs or dfu-util.)
2. A mass-storage device =RPI-RP2= / =NICENANO= appears.
3. Copy =bootymcbootface-rp2040-SIZE.uf2= / =bootymcbootface-nrf52840.uf2= onto it.
4. The board reboots and the bootloader is active.

*** Updating the bootloader itself

Repeat one of the methods above. A new bootymcbootface build replaces the old
